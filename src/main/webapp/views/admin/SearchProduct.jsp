			...
			<div class="container">
				...
				<c:if test="${endPage>1}">
					<div class="product__pagination">
						<c:if test="${tag>1}">
							<c:if test="${tag>16}">
								<a href="searchP?index=1&&txt=${search}"><<</a>
							</c:if>
							<a href="searchP?index=${tag-1}&&txt=${search}"><</a>
						</c:if>
						<c:forEach begin="1" end="${endPage}" var="i">
							<a class="${tag == i ?" active":""}" href="searchP?index=${i}&&txt=${search}">${i}</a>
						</c:forEach>
						<c:if test="${tag<endPage}">
							<a href="searchP?index=${tag+1}&&txt=${search}">></a>
							<c:if test="${tag<endPage-16}">
								<a href="searchP?index=${endPage}&&txt=${search}">>></a>
							</c:if>
						</c:if>
